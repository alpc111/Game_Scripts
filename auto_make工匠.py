...
def make( hwnd ,thing, easy ):
    ...
    click_pos = [244,283,318,352,386,422]
    cur_click_y = 0
    flag = True
    while True:
        ...
        if thing_name_idx > 10000:
            ...
            print thing_name_idx,'click_error'
            flag = False
            break
...
def auto_GetReward( hwnd ):
    KeyClick( hwnd, ord('O') )
    LClickAtPos( hwnd, 285, 506 )
    LClickAtPos( hwnd, 361, 506 )
    LClickAtPos( hwnd, 443, 506 )
    LClickAtPos( hwnd, 535, 506 )
    KeyClick( hwnd, ord('O') )
    KeyClick( hwnd, ord('I') )
    LClickAtPos( hwnd, 704, 144 )
    LClickAtPos( hwnd, 650, 144 )
    LClickAtPos( hwnd, 588, 144 )
    KeyClick( hwnd, ord('I') )



def auto_speak( hwnd, words ):
    KeyClick( hwnd, win32con.VK_RETURN )
    SendChi( hwnd, words )
    KeyClick( hwnd, win32con.VK_RETURN )
...
def close( hwnd ):
    ...
    LClickAtPos( hwnd, 412, 405 )
    LClickAtPos( hwnd, 341, 362 )
...
if __name__ == "__main__":
    ...
    tmp = [ '下品复原灵石', '中品复原灵石', '次级修复宝石', '下品修复宝石', '中品修复宝石',\
            '次级强化宝石','下品强化宝石','中品强化宝石',\
            '天元晶魄','上品复原宝石',\
            '初级高压锅','高级高压锅',\
            '绿松石', '水晶', '黑玛瑙','玄铁石', '紫石英']
    tmp_num = [ 990, 990, 896, 990, 843,\
                482, 777, 548,\
                0,500,\
                990,990,\
                990,990,990,300,0]
    ...
    for x in windows:
        ...
        if ret == name1:
            hwnd = x
            #for y in things[2]:
            #auto_make_Atbusiness( x, things[2], things_num )
                #send_something( x, y, 800 )
    if hwnd != 0:
